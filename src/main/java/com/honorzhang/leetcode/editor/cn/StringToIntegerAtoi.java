...
class Solution {
    public int myAtoi(String str) {
        ...
        while(index < str.length()){
            if (str.charAt(index) == ' ' && flag){
                index++;
                continue;
            }
            if (flag){
                if (str.charAt(index) == '+' || str.charAt(index) == '-' || (str.charAt(index) >= '0' && str.charAt(index) <= '9')){
                    if (str.charAt(index) == '-'){
                        sign = false;
                    }
                    tempStr = str.substring(index, index + 1);
                    flag = false;
                    index++;
                    continue;
                }
